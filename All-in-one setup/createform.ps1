...
'@;
        ...
        Write-ColorOutput Green "Powershell task '$taskName' created: $taskCreateGroupCheckNamesGuid"   
    } else {
        #Get TaskGUID
        $taskCreateGroupCheckNamesGuid = $response.automationTaskGuid
        Write-ColorOutput Yellow "Powershell task '$taskName' already exists: $taskCreateGroupCheckNamesGuid"
    }
} catch {
    Write-ColorOutput Red "Powershell task '$taskName'"
    $_
...
try {
    ...
    if([string]::IsNullOrEmpty($response.dataSourceGUID)) {
        ...
        Write-ColorOutput Green "Task data source '$dataSourceName' created: $dataSourceCreateGroupCheckNamesGuid"
    } else {
        #Get DatasourceGUID
        $dataSourceCreateGroupCheckNamesGuid = $response.dataSourceGUID
        Write-ColorOutput Yellow "Task data source '$dataSourceName' already exists: $dataSourceCreateGroupCheckNamesGuid"
    }
} catch {
    Write-ColorOutput Red "Task data source '$dataSourceName'"
    $_
